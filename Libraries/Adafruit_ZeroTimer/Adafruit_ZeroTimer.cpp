...
Adafruit_ZeroTimer::Adafruit_ZeroTimer(uint8_t timernum, uint8_t gclkgen)
{
  _timernum = timernum;
  _gclkgen = gclkgen;
}
...
/* Array of GLCK ID for different TC instances */
  ...
  uint16_t gclk_clkctrl_gen = (_gclkgen << GCLK_CLKCTRL_GEN_Pos); // Convert from an id (0, 1, 2, etc) into something that fits in the GCLK_CLKCTRL register
  GCLK->CLKCTRL.reg = (uint16_t)(GCLK_CLKCTRL_CLKEN | gclk_clkctrl_gen | inst_gclk_id[instance]);
